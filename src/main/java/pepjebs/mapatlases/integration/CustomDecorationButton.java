...
public class CustomDecorationButton extends DecorationBookmarkButton {
    ...
    public static void renderStaticMarker(PoseStack poseStack, CustomMapDecoration decoration,
        ...
        if (renderer != null) {
            ...
            if (outline) {
                ...
                VertexConsumer vb2 = buffer.getBuffer(R.COLOR_TEXT.apply(texture));
                for(int j = -1; j <= 1; ++j) {
                    for(int k = -1; k <= 1; ++k) {
                        if (j != 0 || k != 0) {
                            poseStack.pushPose();
    ...
    private static class R extends RenderType {
        ...
        public static final Function<ResourceLocation, RenderType> COLOR_TEXT = Util.memoize((p_173249_) ->  create("map_atlases_text_colored",
                DefaultVertexFormat.POSITION_COLOR_TEX_LIGHTMAP,
                com.mojang.blaze3d.vertex.VertexFormat.Mode.QUADS, 256, false, true,

                CompositeState.builder()
                        .setShaderState(SHARD)
                        .setTextureState(new TextureStateShard(p_173249_,
                                false, true))
